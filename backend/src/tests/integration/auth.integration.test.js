...
describe('PATCH /api/auth/profile', () => {
  it('should update user name', async () => {
    ...
    db.query
      ...
      .mockResolvedValueOnce({ rows: [{ id: 1, name: 'Bảo Updated', email: 'bao_integration@foodiego.com', role: 'customer', created_at: new Date() }] }); // SELECT

    const res = await request(app)
      .patch('/api/auth/profile')
      .set('Authorization', 'Bearer mock_jwt_token')
      .send({ name: 'Bảo Updated' });

    expect(res.status).toBe(200);
    expect(res.body.name).toBe('Bảo Updated');
  });
...
describe('POST /api/auth/change-password', () => {
  it('should change password successfully', async () => {
    jwt.verify.mockReturnValueOnce({ id: 1, email: 'bao_integration@foodiego.com', role: 'customer' });
    db.query
      .mockResolvedValueOnce({ rows: [{ password_hash: 'old_hash' }] })
      .mockResolvedValueOnce({ rows: [] }); // UPDATE
    bcrypt.compare.mockResolvedValueOnce(true);
  ...
  it('should return 401 if current password is wrong', async () => {
    jwt.verify.mockReturnValueOnce({ id: 1, email: 'bao_integration@foodiego.com', role: 'customer' });
    db.query.mockResolvedValueOnce({ rows: [{ password_hash: 'old_hash' }] });
    bcrypt.compare.mockResolvedValueOnce(false);
